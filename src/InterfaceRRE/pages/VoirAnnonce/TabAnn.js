...
const TabAnn = () => {
  ...
  function getAnnIds(items) {
    var arr1 = [];
    items.map((items) => arr1.push(items.id_post))
    return arr1;
  }
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        localStorage.removeItem("legthann");
        localStorage.setItem("legthann", getAnnIds(listItems.result));
